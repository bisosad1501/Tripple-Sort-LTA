...
public class BoardGenerator : MonoBehaviour
{
    ...
    private void LoadProduct()
    {
        ...
        for (int i = 0; i < productSetData.productSetList[productIndex].productList.Count; i++)
        {
            ...
            GameObject waresPrefab = Resources.Load("img/object/" + waresId) as GameObject;
            waresPrefabList.Add(waresPrefab);
        }

        productPairCount = currentLv.totalProductCount;

        while (shelfCount * shelfDeepMax < (productPairCount + shelfCount))
        {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (currentSelectWares == null)
            {
                if (IsHitWares())
                {
                    ...
                    {
                        if (GameManager.Instance.uiManager.gameView.tutObj.Phase == 0)
                        {
                            if(currentSelectWares.colID == 0 &&
                               currentSelectWares.shelfController.name == "gm_model_shelve1a")
                            {
                                GameManager.Instance.uiManager.gameView.tutObj.Phase2();
                            }
                            else
                            {
                                GameManager.Instance.uiManager.gameView.tutObj.gameObject.SetActive(false);
                            }
                        }
        ...
        if (Input.GetMouseButtonUp(0))
        {
            if (currentSelectWares != null)
            {
                ...
                if (FindHittedShelf() != null)
                {
                    ...
                    if (GameManager.Instance.currentLevel == 1)
                    {
                        if (GameManager.Instance.uiManager.gameView.tutObj.Phase == 2)
                        {
    ...
    public void ProcessHint()
    {
        ...
        if (waresRemove.Count == 3)
        {
            ...
            GameManager.Instance.uiManager.gameView.UseHIntComplete();
    ...
    private void RemoveHintItems(GameObject hintItem, int itemIndex)
    {
        hintItem.transform.DOMove(new Vector3((itemIndex - 1) * GameManager.Instance.gamePlaySetting.tileSizeX, 9, 35), 0.25f).SetEase(Ease.InQuad).OnComplete
            ...
            () =>
            {
                //hintItem.GetComponent<WaresController>().RemoveItem();
